...
import type { MediaItem, WikidataItem, BibliographyItem, BibliographyProperty, BibliographyRoleType, BibliographicRelationType, AuthorityRelationType, AuthorityEntityType, MediaRelationType, ReferenceLevel, MediaRoleType, LocationItem, NewAnnotation, TagItem } from '@/types/main';
...
const Home: NextPage = () => {
  ...
  // ポリゴンアノテーションの色 + 不透明度（localStorage に永続化）
  const [polygonColor, setPolygonColor] = useState<string>('#facc15');
  const [polygonOpacity, setPolygonOpacity] = useState<number>(0.1);
  useEffect(() => {
    try {
      const saved = localStorage.getItem('polygonStyle');
      if (saved) {
        const { color, opacity } = JSON.parse(saved);
        if (typeof color === 'string') setPolygonColor(color);
        if (typeof opacity === 'number') setPolygonOpacity(opacity);
      }
    } catch { /* ignore */ }
  }, []);
  const updatePolygonStyle = (next: { color: string; opacity: number }) => {
  ...
  // 編集権限：プロジェクトに編集権限があり、かつ自プロジェクトのアノテで、作成者が自分
  ...
  */
  ...
  const refreshObjectAnnotations = async () => {
    ...
    setObjectAnnotationList(anns.map((a) => ({
      ...
      tags: (a as Record<string, unknown>).tags as import('@/types/main').TagItem[] ?? [],
    })));
  };

  // manifest 変更 / showAll 切替時にオブジェクトアノテーション一覧を取得
  useEffect(() => {
  ...
  const openIIIFManifest = async (scope: 'project' | 'all') => {
    ...
    const url = scope === 'project' && projectId
      ...
      : `/api/3/${slug}/manifest`;
    // private プロジェクトを開いている場合は Authorization ヘッダが必要なので
    // window.open ではなく fetch + Blob URL で新規タブを開く。
  ...
  const handleIIIFButtonClick = () => {
    if (!manifestUrl) return;
    if (projectId) {
      setIsIIIFScopeDialogOpen(true);   // 選択ダイアログ
    } else {
      openIIIFManifest('all');           // pid 無しは全件を即開く
    }
  };
        ...
        <div className="flex flex-1">
          <div className="flex-1 border-r border-[var(--border)] relative">
            <ThreeCanvas
              ...
              annotationMode={projectCanEdit ? annotationMode : false}
              manifestUrl={manifestUrl}
              researchProjectId={projectId}
              showAllProjects={showAllProjects}
              editable={projectCanEdit}
            ...
            <div className="absolute top-4 left-4 z-[100] flex flex-col gap-1">
              ...
              <div className="flex flex-col bg-[var(--card-bg)]/95 rounded-lg shadow-lg backdrop-blur-sm border border-[var(--border)] overflow-hidden">
                ...
                <div className="border-t border-[var(--border)] p-1.5 flex items-center justify-center">
                  <PolygonStyleControl
                    color={polygonColor}
                    opacity={polygonOpacity}
                    onChange={updatePolygonStyle}
                  />
                </div>
              </div>
            </div>
