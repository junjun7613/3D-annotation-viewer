...
const Home: NextPage = () => {
  ...
  // manifest has no Range grouping — the switcher then stays a flat pager.
  const canvasGroups = useMemo(() => {
    const groups: { id: string; label: string; indices: number[] }[] = [];
    const byId = new Map<string, number>();
    canvases.forEach((c, i) => {
      if (!c.groupId) return;
      let pos = byId.get(c.groupId);
      if (pos === undefined) {
        pos = groups.length;
        byId.set(c.groupId, pos);
        groups.push({ id: c.groupId, label: c.groupLabel || `Group ${pos + 1}`, indices: [] });
      }
      groups[pos].indices.push(i);
    });
    // A single group conveys no more than the flat list does.
    return groups.length > 1 && groups.every((g) => g.indices.length) ? groups : [];
  }, [canvases]);

  const activeGroupIndex = useMemo(() => {
    if (!canvasGroups.length) return 0;
    const found = canvasGroups.findIndex((g) => g.indices.includes(currentCanvasIndex));
  ...
  */
  ...
  const handlePolygonAnnotation2D = (points: { x: number; y: number }[], canvasId: string) => {
    if (points.length < 3) return;
    setPendingAnnotation({ kind: 'polygon', points, canvasId });
    setPendingTitle('');
  };
  ...
  */
  ...
  return (
    <>
            ...
            <div className="absolute top-4 left-4 z-[100] flex flex-col gap-1">
              <div className="flex flex-col bg-[var(--card-bg)]/95 rounded-lg shadow-lg backdrop-blur-sm border border-[var(--border)] overflow-hidden">
                <button
                  ...
                  className={`p-2.5 transition-colors ${annotationMode === 'none' ? 'bg-[var(--primary)] text-white' : 'hover:bg-[var(--secondary-bg)] text-[var(--text-primary)]'}`}
                  title="Pan / Zoom"
                >
                  <LuMousePointer className="w-4 h-4" />
                </button>
                <button
                  onClick={() => setAnnotationMode(annotationMode === 'rect' ? 'none' : 'rect')}
                  className={`p-2.5 transition-colors border-t border-[var(--border)] ${annotationMode === 'rect' ? 'bg-[var(--primary)] text-white' : 'hover:bg-[var(--secondary-bg)] text-[var(--text-primary)]'}`}
                  title="Rectangle annotation"
                >
                  <LuSquareDashed className="w-4 h-4" />
                </button>
                <button
                  onClick={() => setAnnotationMode(annotationMode === 'polygon' ? 'none' : 'polygon')}
                  className={`p-2.5 transition-colors border-t border-[var(--border)] ${annotationMode === 'polygon' ? 'bg-[var(--primary)] text-white' : 'hover:bg-[var(--secondary-bg)] text-[var(--text-primary)]'}`}
                  title="Polygon annotation"
                >
                  <LuLasso className="w-4 h-4" />
                </button>
              </div>
              {/* Annotation visibility + list */}
              <div className="flex flex-col bg-[var(--card-bg)]/95 rounded-lg shadow-lg backdrop-blur-sm border border-[var(--border)] overflow-hidden">
                <button
      ...
      {/* 2D annotation title input dialog */}
      {pendingAnnotation && (
        <div className="dialog-overlay" onClick={() => setPendingAnnotation(null)}>
          <div className="dialog w-[400px]" onClick={(e) => e.stopPropagation()}>
            <form
              className="flex flex-col gap-4"
              onSubmit={(e) => { e.preventDefault(); savePendingAnnotation(); }}
            >
              <h2 className="text-xl font-bold text-[var(--text-primary)]">New Annotation</h2>
              ...
              <div className="flex justify-end gap-3 mt-2">
                <button type="submit" className="btn-info" disabled={!pendingTitle.trim()}>
                  Save
                </button>
                <button type="button" className="btn-primary" onClick={() => setPendingAnnotation(null)}>
                  Cancel
                </button>
              </div>
            </form>
