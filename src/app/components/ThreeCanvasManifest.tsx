...
interface ThreeCanvasProps {
  ...
  manifestUrl: string;
  onInfoPanelContentChange: (content: { id: string, title: string, description: string, media:[], wikidata:[], bibliography:[] }) => void;
}
...
const getAnnotations = async () => {
  ...
  querySnapshot.forEach((doc) => {
    ...
    const annotationWithId = { ...data, id: doc.id, media: data.media || [], wikidata: data.wikidata || [], bibliography: data.bibliography || [] };
...
const ThreeCanvas: React.FC<ThreeCanvasProps> = ({ annotationsVisible, annotationMode, manifestUrl, onInfoPanelContentChange }) => {
  ...
  const handleInfoPanelContentChange = (content: { id: string, title: string, description: string, media: [], wikidata: [], bibliography:[] }) => {
    setInfoPanelContent(content);
  }

  const handleAnnotationClick = (id:string, title: string, description: string, media: [], wikidata: [], bibliography:[]) => {
    onInfoPanelContentChange({ id,title, description, media, wikidata, bibliography });
  };
            ...
            */
            ...
            const onMouseClick = (event: MouseEvent) => {
              ...
              if (intersects.length > 0) {
                ...
                handleInfoPanelContentChange({ id, title, description, media, wikidata, bibliography });
                handleAnnotationClick(id, title, description, media, wikidata, bibliography);
