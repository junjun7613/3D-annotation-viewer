...
const getAnnotations = async () => {
  ...
  querySnapshot.forEach((doc) => {
    ...
    const annotationWithId = { ...data, id: doc.id, creator: data.creator, media: data.media || [], wikidata: data.wikidata || [], bibliography: data.bibliography || [] };
...
const ThreeCanvas: React.FC<ThreeCanvasProps> = ({ annotationsVisible, annotationMode, manifestUrl, onInfoPanelContentChange }) => {
  ...
  const handleAnnotationClick = (id:string, creator: string, title: string, description: string, media: [], wikidata: [], bibliography:[]) => {
    onInfoPanelContentChange({ id,creator, title, description, media, wikidata, bibliography });
  };
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        const data = doc.data() as Annotation;
        const annotationWithId = { ...data, id: doc.id, creator: data.creator, media: data.media || [], bibliography: data.bibliography || [] };
        annotations.push(annotationWithId);
      });
      // アノテーションの更新を反映
            ...
            */
            ...
            for (let i = 0; i < data.length; i++) {
              ...
              if (annotation.data.target.selector.type == "3DSelector") {
                ...
                sprite.position.set(Number(coordinates[0]), Number(coordinates[1]), Number(coordinates[2]));
            ...
            const onMouseClick = (event: MouseEvent) => {
              ...
              // 交差するオブジェクトを取得
              const intersects = raycaster.intersectObjects([...spritesRef.current, ...polygonsRef.current], true)
              ...
              if (intersects.length > 0) {
                ...
                */
                ...
                handleInfoPanelContentChange({ id, creator, title, description, media, wikidata, bibliography });
                handleAnnotationClick(id, creator, title, description, media, wikidata, bibliography);
            ...
            const onMouseDblClick = (event: MouseEvent) => {
              ...
              if (intersects.length > 0) {
                ...
                if (!annotationModeRef.current) {
                  ...
                  // スプライトの位置をオフセット
                  const spritePosition = intersectedPoint.clone().add(offset || new THREE.Vector3(0, 0, 0));

                  // オフセット後の位置をlookAtに格納
                  lookAt.current = spritePosition;

                  const sprite = new THREE.Sprite(new THREE.SpriteMaterial({ map: new THREE.TextureLoader().load('/images/button.png') }));
                  ...
                  if (polygonVertices.current.length >= 4) {
                    ...
                    polygonArea.current = polygon.geometry.attributes.position.array as Float32Array; // 型を明示的に指定
  ...
  const saveAnnotation = () => {
    ...
    if (annotationModeRef.current) {
      ...
      console.log(polygonArea.current)
      console.log("CamPos: "+camPos.current)
      console.log(title)
      console.log(description)
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        const data = doc.data() as Annotation;
        const annotationWithId = { ...data, id: doc.id, creator: data.creator, media: data.media || [], bibliography: data.bibliography || [], wikidata: data.wikidata || [] };
        annotations.push(annotationWithId);
      });
      // アノテーションの更新を反映
      // 必要に応じて、シーンの再描画や他の処理を行う
